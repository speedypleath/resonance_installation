...
class SmoothVADManager:
    """Manager class for centralized VAD processing that can be used by pipelines."""
    ...
    def __init__(self, output_host="127.0.0.1", output_port=5002,
        ...
        try:
            ...
            print(f"SmoothVADManager: OSC output configured: {output_host}:{output_port}")
        except Exception as e:
            print(f"SmoothVADManager: Could not create OSC client: {e}")
...
def get_smooth_vad_manager(**kwargs):
    """Get or create singleton SmoothVADManager instance."""
    global _smooth_vad_instance
    if _smooth_vad_instance is None:
        ...
        _smooth_vad_instance = SmoothVADManager(**default_params)
    return _smooth_vad_instance

def reset_smooth_vad_manager():
